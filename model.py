...
np.random.seed(np.random.randint(10000) + int(sys.argv[5]))
######################### ---------------------------------------------------------------------------------------------------------------------------------------- #########################
...
class ImpactorPopulation:
	...
	def D_max(self, r, D_c, r_body):
		...
		R_c = D_c/2.0
		R_N = params.continuous_ejecta_blanket_factor*R_c
	...
	def sample_timestep_craters(self, t, avg_imp_diam, primary_lams, max_grid_dist, avg_crater_diam, num_inc, dt, min_crater, resolution, grid_size, grid_width, min_primary_for_secondaries, secondaries_on, r_body, continuous_ejecta_blanket_factor, X_grid, Y_grid):
		...
		np.random.seed(int((time()+t*1000 + int(sys.argv[5]))))
		...
		for i in range(len(avg_imp_diam)):
			...
			if len(primary_crater_diams) > 0:
				...
				for j in range(len(primary_crater_diams)):
					...
					if (0.1*D_p/1.17) >= regolith_thickness and secondaries_on and D_p >= min_primary_for_secondaries:
			...
			if (0.1*avg_crater_diam[i]/1.17) >= regolith_thickness and secondaries_on and avg_crater_diam[i] >= min_primary_for_secondaries:
	...
	def sample_all_craters(self):
		...
		for t in range(nsteps):
			...
			diam_bins = np.array(diam_bins)
			diam_bins_raw = np.copy(diam_bins)
...
class Model:
	def __init__(self):
		params.resolution = float(sys.argv[1])
		params.dt = float(sys.argv[2])
		params.diffusivity = float(sys.argv[3])
		params.grid_size = int(params.grid_width/params.resolution)
		params.dx2=params.dy2 = params.resolution**2
		params.nsteps = int(params.model_time/params.dt)
		params.min_crater = 2.0*params.resolution
		dt_min = 0.1*(params.resolution**2/(4.0*params.diffusivity))
		if params.dt < dt_min:
			params.diff_scheme = 'crank_nicolson'   # Other option here is 'explicit' if you want to give up a little accuracy for a speed improvement
		else:
			params.diff_scheme = 'implicit'
		params.save_dir = sys.argv[4]
	...
	def run(self):
		...
		for t in range(params.nsteps):
			...
			if params.diffusion_on:
				...
				if params.diff_scheme == 'implicit':
					grid_new = grid.implicit_diffusion2D(grid_old)

				elif params.diff_scheme == 'explicit':
					grid_new = grid.explicit_diffusion2D(grid_old)

				elif params.diff_scheme == 'crank_nicolson':
			...
			if params.save_movie:
				t_save = [int((0.5e9)/params.dt), int((1.e9)/params.dt), int((2.0e9)/params.dt), params.nsteps-1]
				if t in t_save:
					np.savetxt('{}/movie_grid_{}.txt'.format(params.save_dir, t), grid_old)
			#gc.collect()
		...
		if params.save_grid:
			fname = str(params.save_dir) + '/Grid/' + str_name

			grid_old = np.array(grid_old, dtype=np.float32)
			np.save(fname, grid_old)

		if params.save_craters:
			fname = str(params.save_dir) + '/Craters/' + str_name
			np.save(fname, [d_craters, t_craters, index_craters])

		if params.tracers_on:
			if params.save_trajectories:
				# Trajectory for all particles (x, y, z, depth, slope)
				trajectory = np.zeros((len(tracers), params.nsteps, 2))

				for j in range(len(tracers)):
					trajectory[j, :, 0] = tracers[j].d_arr
					trajectory[j, :, 1] = tracers[j].slope_arr

				fname = str(params.save_dir) + '/Tracers/' + str_name
